...
class fred_retriever:
    ...
    def __init__(self):
        self.measures = {
            "nasdaqcom": "NASDAQCOM",
            "nyse": "BOGZ1FL073164003Q",
            "willshire": "WILL5000INDFC",
            "gdp": "GDP",
            "gnp": "GNP",
            "unem_rate": "UNRATE",
            "interest_rate": "DFF",
    ...
    def scrape_fred(self, measure):
        ...
        """
        time.sleep(0.7)  # timeout to not overload server
    ...
    def preprocess_data(self, df):
        ...
        df["ECOGROWTH"] = (
            ...
            + 0.7 * df["GDP_change"]
            + 0.6 * df["GNP_change"]
            + 0.5 * df["A027RC1Q027SBEA_change"]
            + 0.4 * df["WILL5000INDFC"]
        )
        df["INFLATION"] = (
            df["PCECTPI"]
            + df["FPCPITOTLZGUSA"]
            + df["NFCILEVERAGE"]
            + df["CORESTICKM159SFRBATL"]
            + df["GDPDEF_change"]
            + df["PPIACO"]
            + df["M2SL"]
        )
