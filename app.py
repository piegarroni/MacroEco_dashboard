...
from dash import Dash, dcc, html, Input, Output, ctx  # pip install dash (version 2.0.0 or higher)
...
"""
# bottoms economy
bottoms = ['04-01-1975', '12-01-1982', '03-01-1992', '08-01-2002', '03-01-2009', '05-01-2020', '02-01-2021', str(date.today())]
...
app.layout = html.Div([
        ...
        html.H1("Visualization Tool for History Economy", style={'text-align': 'center'}),

        dcc.Checklist(id = "select_variables",
            options=[
        ...
        dcc.Graph(id='history_plot', figure={}, style={'width': '100%', 'height': '67vh', 'backgroundColor': 'black'}),
        html.Br(),
        html.Br(),
        html.H1("Visualization Tool for Past Economic Cycles", style={'text-align': 'center'}),
        html.Br(),
        dcc.Dropdown(id="select_cycle",
                    options=[
        ...
        html.Br(),
        dcc.Graph(id='compare_plot', figure={}, style={'width': '100%', 'height': '67vh'}), 
        html.Br(),
        html.Br(),
        ...
        html.Div(
            dcc.RangeSlider(id='select_year',
                            marks={i: {"label": "   " + str(df_dates[i]), "style": {"transform": "rotate(45deg)", 'color':'white'}}
                                        for i in range(0, len(days), 200)},
                            min=0,
                            max=len(days) - 1,
                            value=[0, len(days) - 1],
            ),
            style={"margin-left": "50px", "margin-right": "50px", "margin-top": "25px", "margin-bottom": "25px"} 
        ),

        html.Br(),
        html.Div(id='output'),
        html.Br(),
        dcc.Graph(id='compare_slider_plot', figure={}, style={'width': '100%', 'height': '67vh'}), 
        html.Br(),
        html.Br(),
...
def update_graph(variables, cycle, slider):
    ...
    for column in variables:
        ...
        # Plotly Express
        hist.add_trace(go.Scatter(x=dff.index, y=dff[column], mode='lines', name=column, opacity=0.7))
    ...
    for column in variables:
        ...
        # Plotly Express
        comparison.add_trace(go.Scatter(x=index, y=past_cycle[column], mode='lines', name=f'{column} past', opacity=0.7))
        comparison.add_trace(go.Scatter(x=index, y=present_cycle[column], mode='lines', name=f'{column}  present', opacity=0.7))
 
    # update layout with new title
    comparison.update_layout(title={
    ...
    past_cycle = dff.loc[(dff.index >  dff.index[slider[0]]) & (df.index <  dff.index[slider[1]])]
    present_cycle = dff.loc[(dff.index > bottoms[-2])]
    ...
    for column in variables:
        ...
        # Plotly Express
        comparison_slider.add_trace(go.Scatter(x=index, y=past_cycle[column], mode='lines', name=f'{column} past', opacity=0.7))
        comparison_slider.add_trace(go.Scatter(x=index, y=present_cycle[column], mode='lines', name=f'{column}  present', opacity=0.7))
    
    # update layout with new title
    comparison_slider.update_layout(title={
